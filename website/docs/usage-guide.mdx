...
By default, Node.js caches the imported files. This means that if you change route handler files during the process
runtime, the changes will not be applied even if you re initialise file router.

To avoid this, you can enable the `cleanImportCache` option and reinitialize the router. For example:
