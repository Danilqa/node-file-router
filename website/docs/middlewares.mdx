...
In some situations, you may need to use an object as the final outcome of middleware, or modify one within a request
handler. A good example is altering a Response object in Bun.js. To do this, just return a value from the middleware.
This value will then be the result of the `await next()` call.
